...
static int my_getaddrinfo (char *nodename, int nodeport,
...
my_connect (char *host, int port)
{
  ...
  if ( strlen(host) <= 0 )
    {
      gen_log (1,0, "server specified incorrectly: %s\n", host);
      return 0;
    }

  if ( port < 0 || port > 0xffff )
    {
      gen_log (1,0, "Port specified incorrectly: %d\n", port);
      return 0;
    }
 ...
 ***************************************************************************/
extern int
my_getaddrinfo (char *nodename, int nodeport,
		struct sockaddr *addr, size_t *addrlen)
{
  ...
  inet_addr.sin_port = htons ((unsigned short int) nodeport);
