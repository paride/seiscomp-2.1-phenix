...
string station_name;
enum { Serial_Port, FIFO_Port, TCP_Port } port_type = Serial_Port;
...
class SEEDLog
  ...
  public:
    ...
    SEEDLog(): msglen(PLUGIN_MAX_MSG_SIZE - plugin_name.length() - 5) {}
        
    void operator()(int priority, const string &msg)
      {
        if(!digitime.valid) return;
        
        string msgout = plugin_name + ": " + msg;
...
int open_dev(mode_t mode)
  {
    static const long bpsvalue[] = { 9600, 19200, 38400, 57600, 115200, -1 };
    static const tcflag_t bpsflag[] = { B9600, B19200, B38400, B57600, B115200 };
    int fd, bpsidx;
    struct termios tio;
...
class ChannelDef: public CfgElement
  ...
  public:
    ChannelDef(const string &name): CfgElement(name), first(true) {}
...
try
  {
    ...
    proto->attach_output_channel(source_id, chanid, statid);
...
void configure_plugin(const string &config_file)
  {
    ...
    atts->add_item(IntAttribute("log_soh", dconf.log_soh, 0, 24 * 60 * 60));
