 ...
 * proto_hrd24alt.cc
 *
 * Nanometrics HRD-24 protocol implementation (alternative version)
 *
...
class HRD24Protocol: public Proto
  ...
  private:
    ...
    void process_status_packet(const StatusBundle *b);
...
void HRD24Protocol::process_vcxo_calibration_bundle(const VCXOCalibrationBundle *b)
  {
    ...
    if(b->pll_status == 1 && b->gps_status < 2)
      {
        digitime.quality = 100;
        digitime.exact = true;
      }
    else if(b->pll_status == 1 && b->gps_status < 3)
      {
        digitime.quality = 80;
        digitime.exact = true;
      }
...
RegisterProto<HRD24Protocol> proto("hrd24alt");
