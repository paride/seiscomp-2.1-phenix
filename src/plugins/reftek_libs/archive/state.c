#pragma ident "$Id: state.c,v 1.1.1.1 2005/07/26 19:28:53 andres Exp $"
/* --------------------------------------------------------------------
 Program  : Any
...
BOOL CreateState(CHAR * filespec, STATE * state)
{
    ...
    if ((file = FileOpenForWrite(filespec)) == VOID_H_FILE) {
        ...
        ArchiveLog(ARC_LOG_ERRORS, "FileOpenForWrite failed");
        return (FALSE);
    }

    if (SerializedWrite(state, _state_template, file) == VOID_UINT32) {
        ArchiveLog(ARC_LOG_ERRORS, "SerializedWrite failed");
        return (FALSE);
    }

    if (!FileClose(file)) {
        ArchiveLog(ARC_LOG_ERRORS, "FileClose failed");
        _archive_error = ARC_FILE_IO_ERROR;
        return (FALSE);
    }
...
BOOL OpenStateForWrite(H_ARCHIVE harchive)
{
    ...
    if ((_archive[harchive].file = FileOpenForRead(_archive[harchive].filespec)) == VOID_H_FILE) {
        ArchiveLog(ARC_LOG_ERRORS, "Error opening state file: %s", _archive[harchive].filespec);
    ...
    if (_archive[harchive].state.write) {
        ArchiveLog(ARC_LOG_ERRORS, "Archive is open for write by another process: %s", _archive[harchive].path);
...
BOOL ReadState(H_ARCHIVE harchive)
{
    ...
    if (SerializedRead(&_archive[harchive].state, _state_template,
        ...
        ArchiveLog(ARC_LOG_ERRORS, "Error reading state file (state): %s", _archive[harchive].filespec);
        return (FALSE);
    }
    ...
    for (i = 0; i < _archive[harchive].state.n_streams; i++) {
        ...
        if (SerializedRead(&stream, _stream_template,
                _archive[harchive].file) == VOID_UINT32) {
            ArchiveLog(ARC_LOG_ERRORS, "Error reading state file (stream): %s", _archive[harchive].filespec);
            return (FALSE);
        }
...
BOOL WriteState(H_ARCHIVE harchive)
{
    ...
    if (SerializedWrite(&_archive[harchive].state, _state_template,
            _archive[harchive].file) == VOID_UINT32) {
        ArchiveLog(ARC_LOG_ERRORS, "Error writing state file (state): %s",
    ...
    while (TRUE) {
        ...
        if (SerializedWrite(stream, _stream_template,
                _archive[harchive].file) == VOID_UINT32) {
            ArchiveLog(ARC_LOG_ERRORS, "Error writing state file (stream): %s",
                _archive[harchive].filespec);
            return (FALSE);
        }
