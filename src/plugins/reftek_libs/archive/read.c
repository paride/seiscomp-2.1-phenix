#pragma ident "$Id: read.c,v 1.1.1.1 2005/07/26 19:28:53 andres Exp $"
/* --------------------------------------------------------------------
 Program  : Any
        ...
        if (hdr.type == ET) {
            ...
            while (TRUE) {
                ...
                if (hdr.type == DT && hdr.unit == unit &&
                    ...
                    _reads[hstream].rate = (UINT16) (((REAL64) length / (hdr.time - time)) + 0.5);
                    if (!IsValidRate(_reads[hstream].rate)) {
                        _archive[_reads[hstream].harchive].last_error = ARC_NO_RATE;
                        return (FALSE);
                    }
                    ...
                    ArchiveLog(ARC_LOG_ERRORS, "Warning: sampling rate (%usps) is derived DT times, EH/ET packets will be incomplete!",
                        _reads[hstream].rate);
                    
                    /* Store rate in packet */
                    memset((UINT8 *) (&_reads[hstream].eh) + 24, ' ', sizeof(RF_PACKET) - 24);
                    sprintf(string, "%u", _reads[hstream].rate);
                    for (i = 0; i < 4; i++) {
                        if (string[i])
