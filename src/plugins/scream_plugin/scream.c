...
static char rcsid[] = "$Id: scream.c,v 1.1.1.1 2005/07/26 19:28:54 andres Exp $";

/*
 * $Log: scream.c,v $
 * Revision 1.1.1.1  2005/07/26 19:28:54  andres
 * Imported sources
 *
...
scm_init (int _protocol, char *server, int port)
{
  ...
  char            errstr[255], hostname[63], tstation[15];
  int             socketno, nr, i, try;
  unsigned char   *data;
  int sd;
...
/*This horrible hack is because the protocol version isn't until later...*/
        ...
        gcf_dispatch (buf, GCF_BLOCK_LEN, 0);
