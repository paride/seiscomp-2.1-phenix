...
c  read data
c
      call buf_read(unit,ich,nchannel,mode,mainhead,tracehead)
...
c
      subroutine buf_read
     *(unit,chan_wanted,nchan,mode,main_head,chan_head)
c
...
c
      ...
      character*4240 buffer               ! buffer when reading
      ...
      integer rec_size                    ! number of bytes in one read
...
c   separators, different on sun and pc
c
...
c
      if(computer.eq.'sun'.or.computer.eq.'lin') then
         chan_head=buffer(k+8:k+1047)    ! no separaters
         buf_byte_nr_used=k+1047
      else
         j=1
         do i=1,8
...
c
      if(computer.eq.'sun'.or.computer.eq.'lin') then
        chan_bytes=nsamp*bytes_sample+8
      else    ! pc
...
c
      ...
      if(computer.eq.'sun'.or.computer.eq.'lin') then
         k=1
         do i=buf_byte_nr_used+9,buf_byte_nr_read ! out with record separater
