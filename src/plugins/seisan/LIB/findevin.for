...
c
c   input: base_name :-blank : local data base       
c                     -5 letter name: another data base                     
c                     -local index file name: use that index file               
c                      a local index file is identified to have a '.' or        
c                      be longer than 5 chars.                          
...
c          from_eev   : 1 if call comes from eev, else 0
c                       2 call from ev, over several months
c                                           and output                          
c                                                                               
...
c-- day number in month  and hour         
...
c  type data base, if containing a . or larger than 5 letters must              
...
c   then start from year 1900. If end time in that case is also blank, go to      
...
c
     ...
     *    .and.index_open.eq.1) then               
          if(base_dir.ne.'     ') then   ! a data base   
             write(6,'(1x,i4,1x,i2,1x,$)') year,month
             call sfil
     *       (sdir,slength,sfiles,max_index$,total_no_events)
c
c   update log file
c
...
c                           
      if(index_open.eq.0) then                  
c                                                                               
c   first check if time interval is valid, get abs time from 1900               
c                                                                               
...
c                                                                               
c-- standard type s-file data base             
c
c      write(6,*)' base_dir',base_dir
	  if(base_dir.ne.'     '.and.base_dir(1:2).ne.',,') then     
         ...
         if(year.ge.2099.or.year.lt.1900) then
            write(6,*)'Wrong year'
            stop
         endif
...
c        write(6,*) 'call sfil'
         write(6,'(1x,i4,1x,i2,1x,$)') year,month
         call sfil(sdir,slength,sfiles,max_index$,total_no_events)
c
c   update log file
c
         call base_update(total_no_events,base_dir,year,month)
...
c        write(6,*)'fs,sle',fstart,slength
         ...
         if(i.eq.0.or.(i.eq.1.and.sfiles(1)(1:3).eq.'   ')) then
            ...
            write(6,*)' Number of events in index file: ',
     *      total_no_events
         endif
      endif
c
...
c
        ...
        call get_fstart(evfile,fstart)
      read(evfile(fstart:fstart+4),'(i2,1x,i2)') 
     *current_day,current_hour
c
c   check date and time
c
      if(base_dir.eq.'     '.or.base_dir(1:2).eq.',,') then ! local base or in.
         if(current_day.eq.ev_day_wanted.and.current_hour.ge.
     *      hour_wanted) then   
	        status=0                                                            
	        old_no=event_no                                                     
	        return                                                              
	     ...
	     if(current_day.ge.ev_day_wanted.and.current_hour.ge.
     *      hour_wanted) then                                  
	        status=0                                                            
	        old_no=event_no                                                     
	        return                                                              
...
c   addres is in terms of 4 byte pointers
c
      implicit none
...
c
      if(year.lt.1900.or.year.gt.2049) then
...
c
c     find start of s-file name
c
      implicit none
      integer fstart,sei clen
      character*80 filename
c
c 
      fstart=sei clen(filename)-18
      if(filename(fstart+2:fstart+2).ne.'-') then
