...
5       read(iustat,101,end=1999)v(i),d(i),vs(i),reff
101     format(3f7.3,a1)
...
c    read in trial depth and vp/vs ratio
        read(iustat,'(3f5.0,f5.2,i5,2f5.1)',end=1999)
     *  ztr,xnear,xfar,pos,
...
c
      if(fixdep.ne.'F'.and.fixdep.ne.'S') then
         min_rms= 999999.0
c
c   find if any difference phases that requres initializaiton
...
c
         if(n_start_depths.gt.1) then
            diff_phase=.false.
            do i=1,nphase          
              if(ip(i).eq.9) diff_phase=.true.
            enddo
         endif
c
         do i=1,n_start_depths
            if(n_start_depths.gt.1) then
                if(diff_phase) then   ! rered all from 2. time
                   call getphasn(ks,iazflag,st,phase,pr1,pr,pr3,ip,
     &             tp,fmp,fixdep,depth,fixepi,yres,xres,orgsec,fixor,
     &             iulst,data,test,distind,narriv,imap,xmag,magtype,
     &             oldrms,appar_veloc)
...
c 10/94: added origin time error
     ...
     *  write(iusum,952)idd,thrmin,xhm,la2,ala2,lo2,
     ...
     &  cvxz,cvyz,oterr
952     format(i6,1x,a4,f6.2,i3,1x,f5.2,i4,1x,f5.2,1x,f6.1,
...
c write start depth, xnear, xfar and vpvs to new line type A3
            ...
            write(data(k)(7:12),'(f6.1)') xnear
            write(data(k)(14:19),'(f6.1)') xfar
            write(data(k)(21:24),'(f4.2)') pos
            return
...
2     continue
      ...
      if(text(2:2).eq.' ') then   ! 4 char station
         read(text,103)statid(i)(1:4),la(i),alat(i),ins(i),lo(i),
     &   alon(i),iew(i),ielv(i),dly(i),(mag_cor(k,i),k=1,5) !mag_corr added
     &,   (scrst(i,j),j=1,9)                                !scrst added
         statid(i)(5:5)=' '
      else    ! 5 char station
         read(text,1103)statid(i),la(i),alat(i),ins(i),lo(i),
     &   alon(i),iew(i),ielv(i),dly(i),(mag_cor(k,i),k=1,5) !mag_corr added
     &,   (scrst(i,j),j=1,9)                                !scrst added
      endif
