...
c first try .15 km from closest station 
c     i1=isort(i1) ! thsi line was causing problems to use nearest station
c                  ! comment from barry june 95
      xhtr(1)=x0(i1)+0.00135
      xhtr(2)=y0(i1)+0.00135
      
c need to reset ips(i)=0 for 1st call to rmsv
c      do i=1,nphase
c        ips(i)=0
c      end do

      call depset(xhtr(3),usrc)   ! added aug 2002 by jh+bl
...
c changed 4/94 to not set rmsmin=rms if 'N' phase
      if(ucase(phase(i1)(2:2)).ne.'N'.and.distind.ne.'D'
     & .and.ndata.gt.2)rmsmin=rms
      ...
      if(iulst.gt.0.and.(.not.multi_model))then
        write(iulst,'('' Starting location .15 km from '',
     &  ''closest station: '',5x,3f8.2)')xhtr(2)/degtorad,
     &   xhtr(1)/degtorad
         write(iulst,'('' rms = '',f8.2)')rms
      endif

c  use it if test(56) <= 0 (changed so 0 disables 6/94)
...
c *************** 2.  azimuthal phases  ********************
      ...
      do  i=1,nphase
c        write(iulst,*)st(i),phase(i),ip(i),dtwt(i)
