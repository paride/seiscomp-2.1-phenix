...
c start with initial weights, dtwt
                  if(dtw1(j).gt.0.0)then
                    if(iflagd.eq.0)dtw1(j)=dtwt(j)*wt
                    if(iflagd.eq.1)dtw1(j)=dtwt(j)
                  endif
...
c    layer having a  velocity vl
        if(iflagd.eq.0)dtw1(ii)=dtwt(ii)*wt
...
c            write(*,*)data(1)(1:78)
          ...
          if(phase(ii)(1:4).ne.'    ')write(*,'('' No travel time for ''
