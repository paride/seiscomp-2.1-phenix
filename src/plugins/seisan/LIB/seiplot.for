...
c   plot. The calls will plot on either an X-screen, on PC using microsoft
c   fortran graphics and can at the same time make PostScript files.               
...
c
c   The basic routines on PC are called the same as the original
c   Tektroncis routines. On PC they are linked with Microsoft graphics 
c   while on Sun they can be linked with Tektronics routines, but are
c   from version seisan 6.0 linked to dummy tektronics calls.                  
c                                                          
c                                                                               
...
c               2: only hc file                                                 
c
c   display_type 0: microsoft graphics on pc
c                1: x
c
c   plotunit:   logical file unit for hc file                                   
c   hctype:     0: tektronics file                                              
c               1: postscript                                                   
c                                                                               
...
c                                                                               
      implicit none                                                             
      real x,y                                                                  
      integer ix,iy,ixold,iyold
      include 'seiplot.inc'
      common /movdrw/ixold,iyold
c                                                                               
c   check if tek plot                                                           
c                                                                               
	 if(hctype.eq.0.or.(hctype.gt.0.and.plotoption.lt.2)) then                 
	 ix=x                                                                   
	 iy=y                                                                   
	 if(display_type.eq.0) then
	    call movabs(ix,iy)
	 else
	    call xmoveto(ix,iy)
	    ixold=ix
...
c   postscript                                                                  
c                                                                               
      if(plotoption.gt.0.and.hctype.eq.1) then                                  
...
c                                                                               
	 ...
	 if(display_type.eq.0) then
	     ...
	     call xlineto(ixold,iyold,ix,iy)
	     ixold=ix
	     iyold=iy
	 endif
      endif                                                                     
...
c                                                                               
      if(plotoption.gt.0.and.hctype.eq.1) then                                  
	 write(plotunit,200) x,y                                                
c200     format(2(f7.1,1x),'lineto stroke')       !JAB(BGS)Jun95.
...
c
c   multiply upper loop number with 1.0001 to ensure that exactly
c   200 numbers are read form the stack, rela number inaccuracy
c   did somtimes result in only 199 numbers were read
c
	 write(plotunit,201) 
     *   x0+(i1-1)*step,step,x0+(i2-1)*step*1.0001
 201     format(3(f12.7,1x),'{exch 0.1 mul lineto} for') !JAB(BGS)Jun95.
	 write(plotunit,*)'0.1 setlinewidth'
	 write(plotunit,*)'stroke'
	 i1=i1+200
	 i2=i2+200
	 if(i2.gt.n) i2=n
	 if(i1.gt.n) return                             
cvax         write(plotunit,200) x(i1-1),y(i1-1)
...
c-- distance between points, x-axis                            
      real step                         
      include 'seiplot.inc'
c                                                                               
c   check if tek plot                                                           
...
c-- y-values to stack 
	 write(plotunit,'(16i5)') (int(10*y(i)),i=i2,i1,-1)     
c-- loop values                    
	 write(plotunit,201) x(i1),step,x(i2)           
 201     format(f9.3,1x,f12.7,1x,f9.3,1x,'{exch 0.1 mul lineto} for') !JAB(BGS)Jun95.
	 write(plotunit,*)'0.1 setlinewidth'                                    
	 write(plotunit,*)'stroke'                                              
	 i1=i1+200                                                              
	 i2=i2+200                                                              
...
C
C   PLOT CHARACTERS AT POSITION IX,IY
C
         if(sun.or.linux)CALL MOVABS(IX,IY)
         if(pc) CALL ANSTRPC(string_n,string,IX,IY+18)

cc	   CALL MOVABS(IX,IY+15)
C                                                                               
C   FIND ASCII VALUES OF EACH CHARACTER & plot...
C                                                                               
         if( b_prop_font$ ) then                ! Use non-amended copy.
         string_n = copy_n                      !
         chr_string = chr_copy                  !
         end if                                 !
c
	 DO 10 I=1,string_n 
	 CHAR(I)=ICHAR(SINGLE(I))                                            
 10      CONTINUE                                                               
	 CALL ANSTR(string_n,CHAR)
	 endif
      endif                                                                     
c                                                                               
      RETURN                                                                    
...
C                                                                               
c   J. Havskov, sometimes in 1985                                               
c   1991 by j.h. : adopted to tektronics with real numbers                      
c                                                                               
      implicit none
      CHARACTER*80 STRING                                                       
...
C           
C   FIND ASCI VALUES OF EACH CHARACTER
C           
	 DO 10 I=1,NSTR
	    CHAR(I)=ICHAR(SINGLE(I))
 10      CONTINUE
...
C           
	 if(sun.or.linux)CALL MOVABS(IX,IY)
	 if(pc) call ANSTRPC(NSTR,STRING,IX,IY+15)

c	 CALL MOVABS(IX,IY+15)
	 CALL ANSTR(NSTR,CHAR)
	 endif
      endif                                                                     
c                                                                               
      RETURN                                                                    
...
c----------------------------------------------------------------------------   
      ...
      implicit none                                                             
      integer baud              ! baud rate for tectronics
      integer color_type        ! 1: a color x screen, 0; bw
      include 'seiplot.inc'
      include 'libsei.inc'      !JAB(BGS)Jul95 Library definitions.
      include 'seisan.inc'      
c
      external     computer_type !JAB(BGS)Jul95 Get platform type.
      logical      b_sun,linux !JAB(BGS)Jul95 Sun platform?.
c
c
c get seisan def
c
...
c   find display type
c
      call get_display(display_type)
c
c   Get platform details...
c   -----------------------
c
      call computer_type( b_sun, B_F_PC$,linux )     !JAB(BGS)Jul95 Computer platform.
c
c  in version 6.0, stop if attempt to use Tektronics
c
       if((b_sun.or.linux).and.display_type.eq.0) then
          write(6,*)' Xdisplay not ready'
          stop
       endif
c                                                                               
c   check if tek plot, if only hardcopy, jump next section                     
c                                                                               
      if(hctype.eq.0.or.(hctype.gt.0.and.plotoption.lt.2)) then                 
	 if(disp_open.eq.0) then ! display not already open
	    if(display_type.eq.0) then
	       ...
	       call xopen(wsize,color_type,cursortype,myxfont)   ! x windows
	       if(color_type.eq.0) color_screen=0 ! if not c. screen, turn off col.
	    ...
	    call clear_display
c
c   display might have been changed since last call, reajust
c
	    if(display_type.eq.1) call updatewindowsize()
	 endif
      endif                                                                     
c
c   Determine if messages can be written...
c   ---------------------------------------
c
      B_F_GRAPH$   = disp_open .gt. 0           !JAB(BGS)Jul95 Graphics open?.
      B_F_MESSAGE$ = .not.(B_F_PC$ .and.        !JAB(BGS)Jul95 Can write?.
     &                     B_F_GRAPH$)          !JAB(BGS)Jul95.
c                                                                               
...
c   check if tek plot                                                           
c                                                                               
c      if(hctype.eq.0.or.(hctype.gt.0.and.plotoption.lt.2)) then                 
c         call finitt(k,j)                                                      
c         if(display_type.eq.0) then
c         call finitt(k,j)
c         else
c            call xclose()
c         endif
...
c   postscript                                                                  
c                                                                               
c      if(plotoption.gt.0.and.hctype.eq.1) then                                  
c         write(plotunit,*)'0.2 setlinewidth'                                    
c         write(plotunit,*)'stroke'                                              
c         write(plotunit,*)'showpage'                                            
c         ctld=char(4)                                                           
c         write(plotunit,'(a)') ctld                                             
c      endif                                                                     
c                                                                               
c      return                                                                    
c      end
...
c   check if tek plot                                                           
c                                                                               
      call tsend    ! fix cursor problems on pc
      if(hctype.eq.0.or.(hctype.gt.0.and.plotoption.lt.2)) then                 
	 if(display_type.eq.0) then
	    if(sun.or.linux) call scursr(ich,ix,iy)                                         
	    if(pc)  call sscursr(ich,ix,iy)
	 else
...
c                                                                               
      ...
      subroutine vt
c
c   go to vt  window in X
c
      implicit none
      include 'seiplot.inc'
      logical sun,pc,linux
      call computer_type(sun,pc,linux)
      if(display_type.eq.1) return
      if(sun.or.linux) write(6,*) ' '
      return
      end
c----------------------------------------------------------------
c
c  the escape sequences in routines xt and vt are not what
c  they look like, you cannot just plainly write them. They have
c  been read directly in from ~/COM/xt and vt and must have some
c  hidden stuff inside, see xt and vt on COM
c     
      subroutine xt
c
c  go to tek window in X
c 
      implicit none
      include 'seiplot.inc'
      logical sun,pc,linux
...
c
      call computer_type(sun,pc,linux)
      B_F_PC$ = pc                     !JAB(BGS)Jul 95 Install computer type.
      if(pc) call clear_to_text
      if(sun.or.linux) then
	 if(display_type.eq.1) then
	 ...
	 call finitt(1,1)    ! tek clear
	 call vt             ! back to vt window
      endif
c
...
c
	 ...
	 if(display_type.eq.0) then  ! tektronics
	 endif
	 if(display_type.eq.1) then  ! x
	   call setcolorx(color_to_set)
	 endif
	 if(pc) then  ! pc
	   call set_color_pc(color_to_set)
	 endif
      endif
c                                                                               
...
c
      ...
      include 'seiplot.inc'
	  include 'libsei.inc'
      character*80 colors(60)   ! max number of color objects
...
c
      do i=1,n
	 read(colors(i)(25:25),'(i1)',err=5) c
  5      continue
...
c
     ...
     *color_to_set=xwhite
      if(pc) call set_back_pc(color_to_set)
      if(sun.or.linux) then
...
c   the value canbe set or unset show by inverse video. 
c
...
c 
      ...
      call xchars(title(1:80),80,x0,y0+20+ysize)
      call xchars(title(81:120),40,x0+700.0,y0+20+ysize)
c
...
c  starting in lower left hand cornor x,y
c
      implicit none
      include 'seiplot.inc'
...
C
	 IF( B_FILL ) THEN                       ! Area fill.
         if( plotoption .gt. 0  .and.            ! PostScript fill completion.
     &       hctype     .eq. 1 )                 ! Ditto.
...
c
	...
	call clearwindow(int(ix1),int(iy1),int(ix2),int(iy2))
      call DrawBox(int(ix1),int(iy1),int(ix2),int(iy2))
