...
struct XIM {
    int w, h;
    int ncols;
    char rmap[NCOLS], gmap[NCOLS], bmap[NCOLS];
    char *data;
} xim;
...
FILE *Fp;
    ...
    xim.h = h;
    xim.ncols = numcols;
    memcpy(xim.rmap, rmap, NCOLS);
    memcpy(xim.rmap, rmap, NCOLS);
    memcpy(xim.rmap, rmap, NCOLS);
    xim.data = pic;
    if (!(Fp = fopen("xim.bin", "w")))
    {
        fprintf(stderr,"WriteGIF: can't open 'xim.bin'\n");
        exit(1);
    }
    fwrite(&xim.w,     sizeof(int), 1, Fp);
    fwrite(&xim.h,     sizeof(int), 1, Fp);
    fwrite(&xim.ncols, sizeof(int), 1, Fp);
printf("------ WriteGIF: %d\n", (int) ftell(Fp));
    fwrite(xim.rmap,   sizeof(char), NCOLS, Fp);
    fwrite(xim.gmap,   sizeof(char), NCOLS, Fp);
    fwrite(xim.bmap,   sizeof(char), NCOLS, Fp);
printf("------ WriteGIF: %d\n", (int) ftell(Fp));
    fwrite(xim.data,   sizeof(char), w*h, Fp);
printf("------ WriteGIF: %d\n", (int) ftell(Fp));
    fclose(Fp);
}

if (dbug) printf("==== WriteGIF ptype=%d colorstyle=%d\n",
    ptype, colorstyle);

/* have to quantize down to 8 bits */
/*
  if (ptype == PIC24) {
...
if (dbug) printf(".... %d %d %d\n", rmap[i], gmap[i], bmap[i]);
    ...
    {  /* wasn't found */
      pc2nc[i] = nc;
if (dbug) printf("WriteGIF: color %d\n", pc2nc[i]);
      r1[nc] = rmap[i];
      g1[nc] = gmap[i];
      b1[nc] = bmap[i];
      nc++;
    ...
    {
if (dbug) printf("WriteGIF: found color %d\n", pc2nc[j]);
        pc2nc[i] = pc2nc[j];
    }
  }
...
*/
  ...
  if (dbug) 
    fprintf(stderr,"WriteGIF: pic=%lx, w,h=%dx%d, numcols=%d, Bits%d, Cmapsize=%d\n",
	    (u_long_) pic8, w,h,numcols,BitsPerPixel,ColorMapSize);
