...
int time_jump;
{
  ...
  if (opt.do_time || opt.do_coord || opt.info.on)
  {
      flushInputData(last, time_jump);
      return;
  }

  if (opt.do_sac || opt.do_ah || opt.do_segy || opt.do_bindata)
      OutputData(last, time_jump);

  else if (opt.do_mseed)
      output_miniseed(last, time_jump);

  else if (opt.do_sismalp)
      output_sismalp(last, time_jump);

  else if (opt.do_asc)
      output_asc(last, time_jump);
