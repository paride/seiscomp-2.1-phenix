...
string config_file            = "/home/sysop/config/chain.xml";
...
void get_id(const fsdh_s *fsdh, string &net, string &sta, string &loc,
  string &chn)
  {
...
bool StreamRenamer::hit(fsdh_s *fsdh)
  {
...
class Station
  ...
  private:
    ...
    void rename_streams(fsdh_s *fsdh);
...
void Station::rename_streams(fsdh_s *fsdh)
  {
...
void Station::process_mseed(char *pseed, int packtype, int size)
  {
    ...
    if((q = unpack_map.find(srcname)) != unpack_map.end())
      {
        MSrecord* msr = msr_new();
        msr_parse(NULL, pseed, &msr, 1, 1);
...
class StationGroup
  ...
  public:
    StationGroup(const string &address, bool multi_init, int netto,
      int netdly, int keepalive, int uptime, int standby, int shutdown,
...
StationGroup::StationGroup(const string &address, bool multi_init, int netto,
  int netdly, int keepalive, int uptime, int standby, int shutdown, int seqsave,
  const string &seqfile_init, const string &ifup_init, const string &ifdown_init,
  const string &lockfile_init):
  ...
  ifup_cmd(ifup_init), ifdown_cmd(ifdown_init), multi(multi_init),
        ...
        while((r = sl_collect(slcd, &slpack)) == SLPACKET)
          {
            if(writen_tmo(child_fd, (void *)slpack, SLHEADSIZE + SLRECSIZE,
              WRITE_TIMEOUT) <= 0)
              {
                ...
                fsdh_s* fsdh = (fsdh_s *)(slpack->msrecord);
                string net, sta, loc, chn;
                get_id(fsdh, net, sta, loc, chn);
...
class Chain: private Extension_Partner
  ...
  public:
    ...
    rc_ptr<StationGroup> new_group(const string &address, bool multi,
...
rc_ptr<StationGroup> Chain::new_group(const string &address,
  bool multi, int netto, int netdly, int keepalive, int uptime, int standby,
  int shutdown, int seqsave, const string &seqfile, const string &ifup,
  const string &ifdown, const string &lockfile)
  {
    rc_ptr<StationGroup> group = new StationGroup(address, multi, netto,
      netdly, keepalive, uptime, standby, shutdown, seqsave, seqfile,
...
class GroupElement: public CfgElement
  ...
  public:
    GroupElement(bool multi, int netto, int netdly, int keepalive, int standby,
      int seqsave, const string &overlap_removal):
      CfgElement("group"), def_multi(multi), def_netto(netto),
...
try
  {
    ...
    rc_ptr<StationGroup> group = chain.new_group(address, multi, netto,
...
rc_ptr<CfgElementMap> ChainElement::start_children(ostream &cfglog,
  const string &)
  {
    ...
    elms->add_item(GroupElement(multi, netto, netdly, keepalive, standby,
