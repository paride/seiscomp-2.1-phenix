...
typedef struct
  {
    be_int32_t header_flag ;            /* flag word for header frame */
    ...
    be_int16_t millisec ;               /* time mark millisec binary value */
    be_int16_t time_mark ;              /* sample number of time tag, usually 1 */
    be_int32_t samp_1 ;                 /* 32-bit first sample in record */
    be_int16_t clock_corr ;             /* last clock correction offset in milliseconds */
    be_u_int16_t number_of_samples ;    /* number of samples in record */
    signed char rate ;                  /* samp rate: + = samp/sec; - = sec/samp */
    byte blockette_count ;
    time_array time_of_sample ;         /* time of the tagged sample */
    be_int32_t packet_seq ;
    byte header_revision ;
    byte detection_day ;
    be_int16_t detection_seq ;
 /* Following fields are valid in QSL format only */
    be_int32_t clock_drift ;
    seed_name_type seedname ;
    signed char clkq ;
    seed_net_type seednet ;
    location_type location ;
    be_int16_t ht_sp3 ;
    be_int16_t microsec ;               /* time mark microsec binary value */
    byte frame_count ;
    byte hdrsp1 ;
    byte z[5] ;
    byte ht_sp4 ;
...
typedef struct
  {
    be_int32_t jdate ;                  /* seconds since Jan 1, 1984 00:00:00 */
    be_int16_t millisec ;               /* 0..999 fractional part of time */
    byte component ;                    /* not valid in QSL */
    byte stream ;                       /* not valid in QSL */
    be_int32_t motion_pick_lookback_quality ; /* "string" of this gives, eg "CA200999" */
    be_int32_t peak_amplitude ;         /* & threshold sample value */
    be_int32_t period_x_100 ;           /* 100 times detected period */
    be_int32_t background_amplitude ;   /* & threshold limit exceeded */
  /* Following fields are valid in QSL Format only */
...
typedef struct
  {
    be_int32_t msec_correction ;       /* last timemark offset in milliseconds */
...
typedef struct
  {
    ...
    be_int32_t count_of ;                  /* seconds elapsed or consecutive marks */
    byte correction_quality[6] ;
    be_int16_t usec_correction ;           /* timemark offset, 0 - 999 microseconds*/
    be_int32_t clock_drift ;               /* clock drift in microseconds */
    be_int16_t vco ;                       /* full VCO value */
...
typedef struct
  {
    byte frame_type ;
    byte word_align ;
    be_int16_t cr_duration ;           /* duration in seconds NOT LONG ALIGNED -LONG*/
    be_int16_t cr_duration_low ;
    be_int16_t cr_period ;             /* period in milliseconds NOT LONG ALIGNED-LONG */
    be_int16_t cr_period_low ;
    be_int16_t cr_amplitude ;          /* amplitude in DB */
...
/* Following are valid in QSL format only */
    ...
    be_int16_t cr_0dB ;                /* 0 = dB, <> 0 = value for 0dB NOT LONG ALIGNED-FLOAT */
    be_int16_t cr_0dB_low ;
...
typedef struct
  {
    ...
    byte frame_type ;
    boolean first_seg ;
    be_u_int16_t total_bytes ;
    be_int16_t total_seg ;
    be_int16_t this_seg ;
    be_u_int16_t byte_offset ;
    be_u_int16_t byte_count ;
...
typedef struct
  {
    ...
    byte frame_type ;
    boolean rcecho ;
    be_int16_t seq_modulus ;
    be_int16_t window_size ;
    byte total_prio ;
    byte msg_prio ;
    byte det_prio ;
    byte time_prio ;
    byte cal_prio ;
    byte link_format ;
    be_int16_t resendtime ;  /* resend packets timeout in seconds */
    be_int16_t synctime ;    /* sync packet time in seconds */
    be_int16_t resendpkts ;  /* packets in resend blocks */
    be_int16_t netdelay ;    /* network restart delay in seconds */
    be_int16_t nettime ;     /* network connect timeout in seconds */
    be_int16_t netmax ;      /* unacked network packets before timeout */
    be_int16_t groupsize ;   /* group packet count */
    be_int16_t grouptime ;   /* group timeout in seconds */
   } commo_link ;
...
typedef struct
  {
    be_int16_t chksum ;   /* chksum not calc over error pkt */
    be_int16_t crc ;      /* first gen crc, then make checksum NOT LONG ALIGNED-LONG */
    be_int16_t crc_low ;
...
typedef struct
  {
    be_int16_t skipped ;     /* START FILLING BUFFER AT "seq", THIS ASSURES THAT
...
typedef struct
  {
    digi_record digi ;
    be_int16_t vcovalue ;           /* current vco value */
    boolean pllon ;                 /* true if PLL controlling PLL */
    boolean umass_ok ;              /* coyp of cal.mass_ok */
    be_int16_t comcount ;           /* number of comm events */
    be_int16_t comoffset ;          /* offset to start of comm names */
    be_int16_t usedcount ;          /* number of used combos */
    be_int16_t usedoffset ;         /* offset to start of used combinations */
    be_int16_t calcount ;           /* copy of cal.number */
    be_int16_t caloffset ;          /* offset to start of calibrator defs */
    byte cpr_levels ;               /* comlink priority levels */
    byte ultra_rev ;                /* revision level, current = 1 */
    be_int16_t ut_sp2 ;
    be_int32_t comm_mask ;          /* current comm detector mask */
  } ultra_type ;
