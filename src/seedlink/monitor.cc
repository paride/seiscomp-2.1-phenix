...
const int  MAXSEL       = 50;   // maximum number of selectors per connection
...
int packet_type(const fsdh_s *fsdh, int size)
  {
    int b2000 = 0;
    const blkt_head_s *p = (const blkt_head_s *)((const char *)(fsdh) +
    ...
    do
      {
        ...
        p = (const blkt_head_s *)((const char *)(fsdh) + next_blkt);
      }
    while((fsdh_s *)p != fsdh);
...
double packet_sample_rate(const fsdh_s *fsdh)
  {
...
INT_TIME packet_begin_time(const fsdh_s *fsdh)
  {
    EXT_TIME et;
...
INT_TIME packet_end_time(const fsdh_s *fsdh)
  {
...
StreamDescriptor make_stream_descriptor(const fsdh_s *fsdh, int size)
  {
    int type;
...
bool ConnectionMonitorImpl::match_packet(const void *head, int size)
  {
    ...
    const fsdh_s* fsdh = static_cast<const fsdh_s *>(head);
